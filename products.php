...
if(isset($_GET["delete"])){
    ...
    try {
        ...
        // Delete from stock table first (due to foreign key constraint)
        $sql_stock = "DELETE FROM stock WHERE product_id = ?";
        if($stmt_stock = mysqli_prepare($conn, $sql_stock)){
        ...
        // Delete from sale_details table
        $sql_sale_details = "DELETE FROM sale_details WHERE product_id = ?";
        if($stmt_sale_details = mysqli_prepare($conn, $sql_sale_details)){
            mysqli_stmt_bind_param($stmt_sale_details, "i", $id);
            mysqli_stmt_execute($stmt_sale_details);
            mysqli_stmt_close($stmt_sale_details);
        }
        
        // Delete from purchase_details table
        $sql_purchase_details = "DELETE FROM purchase_details WHERE product_id = ?";
        if($stmt_purchase_details = mysqli_prepare($conn, $sql_purchase_details)){
            mysqli_stmt_bind_param($stmt_purchase_details, "i", $id);
            mysqli_stmt_execute($stmt_purchase_details);
            mysqli_stmt_close($stmt_purchase_details);
        }
